...
- **Autocomplete Inteligente e Prevenção de Duplicidade**: Campos-chave (como "Nome Completo") devem operar como um Autocomplete (com debounce para a API) para acelerar a busca visual (ex: exibir `Nome - Modelo do Veículo (Cor) - Placa`). Ao clicar na sugestão, o formulário deve atuar proativamente impedindo uma submissão cega, redirecionando o usuário para o cadastro existente, mantendo bypass normal caso sejam homônimos.
...
## 5. Testes E2E (Playwright) e Sincronicidade

- **Armadilha do Debounce**: Quando interagir com inputs atrelados a Autocompletes com _debounce_ (ex: 500ms), **jamais** clique no botão de "Salvar/Submeter" logo após preenchê-los. A API será disparada assincronamente e o teste tentará salvar com o _React Hook Form_ num estado transiente (o que bloqueia a submissão silenciosamente). Sempre injete um `await page.waitForTimeout(1000);` ou espere ativamente a promessa de rede correspondente (`waitForResponse`) antes do evento de `.click({ force: true })`.
- **A Armadilha do waitForResponse**: O clique disparador deve ocorrer entre a declaração da promessa do `.waitForResponse` e a resolução (`await promise`), utilizando obrigatoriamente `.click({ force: true })` para prevenir que Toasts anteriores sobrepostos bloqueiem a ação.
- **Extração Segura e Isolamento**: Em suítes declaradas como `test.describe.serial` que compartilham IDs gerados (estado global), a extração deve sempre validar ativamente se o objeto recebido não é nulo antes de persistir nas variáveis. Caso contrário, uma instabilidade de rede em um teste de criação quebra toda a cadeia subsequente.
- **Modais Fade-In (HeadlessUI/Radix)**: Ao lidar com modais (como confirmação de exclusão), evite invocar cliques imediatos após o disparo de abertura. Sempre selecione o contêiner único (ex: `.fixed.inset-0.z-50`), aguarde `.waitFor({ state: 'visible' })` e utilize um pequeno delay de UI (ex: 300ms) para acomodar a opacidade css (fade-in) antes do clique na confirmação.

6. Integridade de Dados e Proteção de Histórico
   Sanitização de Fronteira (Data Cleansing): O banco de dados armazena CPFs, CNPJs, CEPs e Telefones estritamente como números. A remoção de máscaras visuais (ex: replace(/\D/g, '')) deve ocorrer obrigatoriamente no Backend antes da persistência no Repository, garantindo a padronização para integrações futuras (NFe, WhatsApp).

Exclusão Lógica (Soft Delete) e Efeito Cascata: É terminantemente proibida a exclusão física (DELETE) de clientes, veículos ou peças. O sistema opera exclusivamente com a flag ativo: false para preservar a integridade referencial das Ordens de Serviço e do fluxo de caixa. A inativação de um cliente obriga a inativação imediata (em cascata) de todos os veículos e equipamentos a ele vinculados.
