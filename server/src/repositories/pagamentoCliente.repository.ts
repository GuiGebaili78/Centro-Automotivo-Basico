...
export class PagamentoClienteRepository {
  async create(data: Prisma.PagamentoClienteCreateInput) {
    return await prisma.$transaction(async (tx) => {
      ...
      if (metodo === "PIX" && idConta) {
        ...
        console.log(
          `✅ [LC] PIX Imediato Criado: LC #${livroCaixa.id_livro_caixa} | Conta #${idConta} (+${p.valor})`,
        );
      }
      ...
      // CASO 2: DINHEIRO (Vai SÓ pro Livro Caixa, SEM Banco)
        ...
        console.log(
          `✅ [LC] DINHEIRO Imediato Criado: LC #${livroCaixa.id_livro_caixa} (Sem impacto bancário)`,
        );
      }
  ...
  async update(id: number, data: Prisma.PagamentoClienteUpdateInput) {
    return await prisma.$transaction(async (tx) => {
      ...
      const oldPayment = await tx.pagamentoCliente.findUnique({
        where: { id_pagamento_cliente: id },
      });
      ...
      if (oldP.id_livro_caixa) {
        ...
        if (oldP.id_conta_bancaria) {
          ...
          console.log(
            `↺ [LC] Reversão Pagamento #${id}: Conta #${oldP.id_conta_bancaria} (-${oldP.valor})`,
          );
        }
      ...
      if (metodo === "PIX" && idConta) {
        const livroCaixa = await tx.livroCaixa.create({
          data: {
            ...
            obs: finalObs || null, // Save OBS here
          },
        });
        ...
        console.log(
          `✅ [LC] PIX Atualizado: LC #${livroCaixa.id_livro_caixa} | Conta #${idConta} (+${newP.valor})`,
        );
      }
      ...
      // CASO 2: DINHEIRO (Só LC)
        ...
        console.log(
          `✅ [LC] DINHEIRO Atualizado: LC #${livroCaixa.id_livro_caixa} (Sem impacto bancário)`,
        );
      }
  ...
  async delete(id: number) {
    return await prisma.$transaction(async (tx) => {
      const payment = await tx.pagamentoCliente.findUnique({
        where: { id_pagamento_cliente: id },
      });
      ...
      if (p.id_livro_caixa) {
        if (p.id_conta_bancaria) {
          ...
          console.log(
            `🗑️ [LC] Pagamento Excluído/Estornado #${id}: Conta #${p.id_conta_bancaria} (-${p.valor})`,
          );
        }
