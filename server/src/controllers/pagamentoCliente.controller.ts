      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (
          ...
          if (data.metodo_pagamento === "DEBITO") {
            ...
            if (data.tipo_parcelamento === "CLIENTE") {
              // Cliente assume juros -> Valor entra cheio (sem descontar taxa da oficina)
              taxa = 0;
            } else {
              // Loja assume juros -> Buscar taxa específica para o nº de parcelas
              const taxaEspecial = await tx.taxaCartao.findUnique({
                where: {
                  id_operadora_modalidade_num_parcelas: {
              ...
              if (taxaEspecial) {
                taxa = Number(taxaEspecial.taxa_total);
              } else {
