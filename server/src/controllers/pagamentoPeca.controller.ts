...
export class PagamentoPecaController {
  ...
  async update(req: Request, res: Response) {
    try {
      const id = Number(req.params.id);
      const { pago_ao_fornecedor, id_conta_bancaria, data_pagamento_fornecedor } = req.body;
      ...
      if (pago_ao_fornecedor === true && id_conta_bancaria) {
         try {
             const date = data_pagamento_fornecedor ? new Date(data_pagamento_fornecedor) : new Date();
             const result = await repository.confirmPayment(id, Number(id_conta_bancaria), date);
             return res.json(result);
         } catch (e: any) {
             console.error("Confirm failed", e);
             return res.status(400).json({ error: e.message || 'Failed to confirm payment' });
         }
      }
