    ...
    const loadData = async () => {
        try {
            ...
            const outflows = paymentsRes.data
                .filter((p: any) => (p.pago_ao_fornecedor && p.data_pagamento_fornecedor) || p.deleted_at)
                .map((p: any) => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 relative">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-xl font-bold text-neutral-800">Histórico de Movimentações</h2>
                    <p className="text-neutral-500 text-sm">Registro completo de entradas e saídas.</p>
                </div>
                <button 
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-neutral-50">
                            {filteredCashBook.length === 0 ? (
                                 <tr><td colSpan={8} className="p-10 text-center text-neutral-400 italic font-medium">Nenhuma movimentação encontrada.</td></tr>
                            ) : (
